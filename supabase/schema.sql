...
check (status in ('available', 'booked', 'borrowed', 'in_transit', 'repair', 'maintenance', 'suspended', 'employee_car', 'deregistered', 'sold', 'retired'));
...
create policy "Admins can insert allowed emails" on public.allowed_user_emails
for insert to authenticated with check (public.is_admin() and created_by_user_id = auth.uid());
drop policy if exists "Admins can delete allowed emails" on public.allowed_user_emails;
create policy "Admins can delete allowed emails" on public.allowed_user_emails
for delete to authenticated using (public.is_admin());

create or replace function public.is_signup_email_allowed(p_email text)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.allowed_user_emails where email = lower(trim(p_email)));
$$;
revoke all on function public.is_signup_email_allowed(text) from public;
revoke all on function public.is_signup_email_allowed(text) from anon, authenticated;
...
create or replace function public.consume_auth_rate_limit(p_key text, p_action text, p_max_attempts integer, p_window_seconds integer)
