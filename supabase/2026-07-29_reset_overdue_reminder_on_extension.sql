-- Reset the overdue reminder marker atomically whenever a borrower extends a loan.
...
begin
  ...
  if exists (
    ...
    where b.vehicle_id = v_loan.vehicle_id
      and tstzrange(
        b.starts_at,
        case when b.is_long_term then 'infinity'::timestamptz else b.ends_at end,
        '[)'
      ) && tstzrange(v_loan.borrowed_at, p_expected_return_at, '[)')
  ) then
